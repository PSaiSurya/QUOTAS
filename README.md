...
**QUOTAS** provides a fully automated, Dockerized testbed to measure the exact latency, variance, and Denial-of-Service (DoS) thresholds of **Modbus TCP, S7comm, and OPC UA** when routed through Quantum-Safe VPN tunnels under highly constrained CPU limits.
...
- **Zero Steady-State Impact:** These grids visually map the overhead of NIST algorithms against the classical baseline. Notice the drastic shift between the Micro RTU and the others: because PQC relies on asymmetric handshakes, the heavy mathematical lifting occurs _exclusively_ during VPN initialization. Once established, steady-state symmetric polling (AES-GCM) induces no practically significant latency penalty above the 10% starvation boundary. At 150% CPU, Welch's t-tests confirm statistical parity across all five PQC suites (p = 0.133–0.265, Cohen's |d| ≤ 0.39). At 50% CPU, Level 3–5 suites show marginal statistical significance (p = 0.014–0.021); however, the maximum mean latency delta against the Classical Baseline at that tier remains below 0.51 ms — operationally negligible within standard SCADA polling cycles.
...
### 5. Statistical Validation of Steady-State Operational Parity

Welch's two-sample t-tests were computed against the empirical CSV (`nist_pqc_ot_benchmarks.csv`) at the 150% CPU tier (n = 30 per group), the unconstrained operating condition depicted in the High-End PLC overhead chart above. All p-values exceed the α = 0.05 threshold; effect sizes are uniformly small.
| Suite | n | Mean ± SD (ms) | t (df) | p-value | Cohen \|d\| |
|---|---|---|---|---|---|
| Classical Baseline | 30 | 1.855 ± 0.645 | — | — | — |
| NIST Level 1 (ML-DSA-44 / ML-KEM-512) | 30 | 1.625 ± 0.515 | 1.52 (55.3) | 0.133 | 0.39 |
| NIST Level 3 (ML-DSA-65 / ML-KEM-768) | 30 | 2.065 ± 0.703 | −1.21 (57.6) | 0.232 | 0.31 |
| NIST Level 5 (ML-DSA-87 / ML-KEM-1024) | 30 | 2.049 ± 0.651 | −1.16 (58.0) | 0.251 | 0.30 |
...
<br>
This repository uses Git Large File Storage (LFS) for the compiled Docker archives. First, clone the repository, install Git LFS to pull the actual binary files, and configure your Python virtual environment:
...
# Install Git LFS and pull the large Docker image archive
sudo apt-get update
